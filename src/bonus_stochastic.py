...
def computeCost(X, y, w, b, lambda_ = 1):
    ...
    cost_hufflepuff = 0
    for i in range(m):
        z_slytherin = np.dot(w_slytherin, X.iloc[i].item) + b_slytherin
        ...
        z_ravenclaw = np.dot(w_ravenclaw, X.iloc[i].item) + b_ravenclaw
        ...
        z_gryffindor = np.dot(w_gryffindor, X.iloc[i].item) + b_gryffindor
        ...
        z_hufflepuff = np.dot(w_hufflepuff, X.iloc[i].item) + b_hufflepuff
...
def updateWb(X, y, w, b, lambda_, alpha):
    ...
    # # n = len(X.iloc[0,:])  # Nombre de features
    # # print("lol")
    # # dj_dw = np.zeros((4, n))
    # # dj_db = np.zeros(4)
    # # print("lol")

    # # Décompactez les poids et les biais
    # # w_slytherin, w_ravenclaw, w_gryffindor, w_hufflepuff = w
    # # b_slytherin, b_ravenclaw, b_gryffindor, b_hufflepuff = b

    # # Décompactez les labels
    # # y_slytherin, y_ravenclaw, y_gryffindor, y_hufflepuff = y

    # # Calcul du coût et des erreurs
    # # for house in range(4):
    # #     z = np.dot(w[house], X) + b[house]
    # #     f_wb = 1 / (1 + np.exp(-z))
    # #     err = f_wb - y[house]
    ...
    # # Mise à jour des gradients régularisés
    # # for house in range(4):
    # #     dj_dw[house] += (lambda_ / len(X)) * w[house]
...
def executeGradientDescentAlgo(X, y, alpha, lambda_, nb_iterations):
    ...
    # result[0][i], result[1][i], result[2][i], result[3][i] = computeCost(X.iloc[i, :], y, w, b, lambda_)
    return(w, b, result)
